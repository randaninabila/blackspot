...
<div class="max-w-4xl mx-auto px-4 py-10">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <h1 class="text-3xl font-bold text-[#234B26]">Detail Validasi</h1>
    </div>
        ...
        <div class="bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-sm h-fit">
            <table class="w-full text-sm">
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr>
                        <td class="bg-gray-50 px-4 py-3 font-bold text-[#234B26]">Dilaporkan Oleh</td>
    ...
    <div class="flex gap-4 mt-6 flex-wrap">
        ...
        <form action="{{ route('admin.validasi.setujui', $blankSpot->id) }}" method="POST" onsubmit="return confirm('Setujui data ini?')">
            @csrf
            <button type="submit" class="bg-[#234B26] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#1a381c] transition">
                ✅ Setujui
            </button>
        </form>
        @endif
        ...
        <form action="{{ route('admin.validasi.tolak', $blankSpot->id) }}" method="POST" onsubmit="return confirm('Tolak data ini?')">
            @csrf
            <button type="submit" class="bg-red-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-red-700 transition">
                🚫 Tolak
            </button>
        </form>
        @endif

        <a href="{{ route('admin.validasi.edit', $blankSpot->id) }}"
...
@push('scripts')
<script>
const vmap = L.map('valMap').setView([{{ $blankSpot->latitude }}, {{ $blankSpot->longitude }}], 14);
...
L.marker([{{ $blankSpot->latitude }}, {{ $blankSpot->longitude }}])
    .addTo(vmap)
    .bindPopup(`<b>{{ $blankSpot->kabupaten->nama_kabupaten ?? '' }}</b><br>{{ $blankSpot->kecamatan->nama_kecamatan ?? '' }}`)
    .openPopup();
</script>
@endpush
@endsection